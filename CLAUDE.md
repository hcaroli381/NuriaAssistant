...
- **The exchange must redeem the code against the same redirect URI the QR authorized with.** Spotify binds the code to it and answers `INVALID_CLIENT`/400 otherwise; the library defaults the token request to the URI configured on the API client (the loopback one), so `SpotifyService` remembers the URI each authorize URL was built with (`codeRedirectUri`) and reuses it in `buildCodeExchange`. `SpotifyServiceTest` asserts the value on the wire, not just the remembered field.
- `handleSpotifyCallback` blocks until the exchange answers and returns its result: the phone's page is written from that value, so it can no longer claim *Spotify conectado* while no token was stored (an early `return true` did exactly that, which is how the QR silently "worked" and connected nothing).
